...
const LoginScreen = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Sem NEXT_PUBLIC_SUPABASE_URL/ANON_KEY configuradas, cai para o fluxo
            // legado (JWT próprio + cookie) para não bloquear o login local.
            if (!isSupabaseConfigured) {
                if (activeTab === 'entrar') {
                    ...
                    const res = await api.post('/auth/signup', { name, email, password, nickname, position });
                    if (res.data.error) {
            ...
            toast.error(error.response?.data?.error || error.message || 'Erro na conexão com o servidor');
        } finally {
            setLoading(false);
        }
    };
                    ...
                    >
                        ...
                        <form onSubmit={handleForgotPassword} className="space-y-4">
                            <Input
                                ...
                                icon={Mail}
                            />
                            <Button type="submit" fullWidth size="lg" isLoading={forgotLoading}>
                                Enviar link
                            </Button>
                        </form>
                    </div>
